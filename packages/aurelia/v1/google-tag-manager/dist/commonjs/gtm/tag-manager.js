...
const LogManager = tslib_1.__importStar(require("aurelia-logging"));
const configure_1 = require("./configure");
class TagManager {
    constructor() {
        this.ea = (0, aurelia_dependency_injection_1.resolve)(aurelia_event_aggregator_1.EventAggregator);
        this.settings = (0, aurelia_dependency_injection_1.resolve)(configure_1.Configure);
    ...
    enable() {
        ...
        if (this.options.trackCurrentPageOnEnable)
            this.trackPage(aurelia_pal_1.PLATFORM.global.location.pathname, aurelia_pal_1.DOM.title);
    }
    disable() {
    ...
    attachScriptElements(key) {
        ...
        if (!this.scriptElement) {
            const scriptElement = aurelia_pal_1.DOM.createElement('script');
            ...
            aurelia_pal_1.DOM.querySelector('head').appendChild(this.scriptElement);
        }
        if (!this.noScriptElement) {
            const noscriptElement = aurelia_pal_1.DOM.createElement('noscript');
            const iframeElement = aurelia_pal_1.DOM.createElement('iframe');
            ...
            const body = aurelia_pal_1.DOM.querySelector('body');
    ...
    resetDataLayer() {
        const gtm = aurelia_pal_1.PLATFORM.global.google_tag_manager[this.options.key];
