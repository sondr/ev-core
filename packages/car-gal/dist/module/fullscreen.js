...
export class Fullscreen {
    ...
    show(index) {
        if (this.options.Fullscreen.Events.onShow)
            this.options.Fullscreen.Events.onShow();
        ...
        if (typeof index === _TYPES.number)
            this.carousel.set_active(index);
    }
    close() {
        if (this.options.Fullscreen.Events.onClose)
            this.options.Fullscreen.Events.onClose();
    ...
    setThumbnailsActiveState(value) {
    ...
    setMediaInfo(media, index, length, carousel) {
        if (!media.element)
            media.element = findElement(media.containerElement, _HTML.Tags.img);
